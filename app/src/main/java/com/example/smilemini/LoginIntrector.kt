...
open class LoginIntrector : LoginContract.Interactor {
    ...
    override fun proceed(email: String, pass: String): Observable<BasePartialChanges> {
        val req = RetrofitClass.retrofit.create(AuthApi::class.java).getUser(email,pass,"2.0").map {
            it.raw()
        }
...
//        else{
        ...
        return RetrofitClass.retrofit.create(AuthApi::class.java).getUser(email,pass,"2.0").map  {
         ...
         // val bdy=it.body().toString()
           //  Log.d("body",bdy)
             ...
             /* val  req1= RetrofitClass.retrofit.create(GettingInformationApi::class.java)
               val request3nd =req1.getUserDetails(tokens.getLong("customerId").toString(),"id")
                *//*.observeOn(AndroidSchedulers.mainThread())
                   .subscribeOn(Schedulers.io())*//*
                request3nd.subscribe {
